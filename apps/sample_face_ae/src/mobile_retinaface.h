...
class MobileRetinaface : public Model {
 public:
  MobileRetinaface(const char *kmodel_file, size_t channel, size_t height,
                   size_t width);
  ~MobileRetinaface();
  DetectResult get_result() const { return result_; }
 ...
 private:
  ...
  void deal_conf_opt(float *conf, float *s_probs, int *s, int size,
                     int *obj_cnt, int *real_count, float *tmp);
  void deal_loc_opt(float *loc, float *boxes, int size, int *obj_cnt, int *s,
                    int *real_count);
  void deal_landms_opt(float *landms, float *landmarks, int size, int *obj_cnt,
                       int *s, int *real_count);
  box_t get_box_opt(float *boxes, int obj_index, int index_anchors);
  landmarks_t get_landmark_opt(float *landmarks, int obj_index,
                               int index_anchors);
